...
const LanguageSwitcher = () => {
  ...
  return (
    // 2. Wrapper harus 'relative'
    <div className="relative w-full max-w-[160px]">
      {/* 3. Tombol 'Trigger' */}
      <button
        type="button" // PENTING: Tambahkan type="button" agar tidak submit form tidak sengaja
        onClick={() => setIsOpen(!isOpen)}
        className="h-[50px] w-full gap-2 text-[16px] font-normal pl-4 sm:pr-4 rounded-2xl  flex items-center justify-between text-gray-700  transition-colors border border-transparent "
      >
        <span className="flex items-center gap-3">
          {/* Ini adalah bendera di tombol */}
        ...
        <ul className="absolute z-50 w-full bg-white shadow-xl border border-gray-100 rounded-2xl mt-2 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          {languages.map((lang) => (
            <li
              key={lang.value}
              // --- PERHATIKAN DI SINI ---
              // Harus pakai () => ... agar tidak infinite loop
              ...
              `}
            >
              {/* Ini adalah bendera di daftar pilihan */}
